...
import { rejectChangeRequestSchema } from "@/lib/change-management/schema";
import { rejectChangeRequest } from "@/lib/change-management/service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const details = await rejectChangeRequest(id, access.principal, parsed.data.comment);
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Unable to reject Change Request." },
      { status: 400 }
    );
  }
}
