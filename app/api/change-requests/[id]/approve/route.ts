...
import { decideChangeRequestSchema } from "@/lib/change-management/schema";
import { approveChangeRequest } from "@/lib/change-management/service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = decideChangeRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const details = await approveChangeRequest(id, access.principal, parsed.data.comment);
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Unable to approve Change Request." },
      { status: 400 }
    );
  }
}
