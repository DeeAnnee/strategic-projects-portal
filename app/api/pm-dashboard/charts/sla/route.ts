...
export async function GET(request: Request) {
  try {
    ...
    const payload = await getPmDashboardSlaCharts(toRbacPrincipal(access.principal), filters);
    return NextResponse.json({ data: payload });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to load PM dashboard SLA charts.";
