...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return persistenceErrorResponse(error, "Failed to load project for need-more-info.");
  }
  if (!submission) {
  ...
  let updated: ProjectSubmission | null = null;
  try {
    ...
    if (resolvedStage === "PROJECT_MANAGER") {
      ...
      updated = await recordProjectApprovalDecision(id, {
        stage: resolvedStage,
    ...
    if (isDataStorePersistenceError(error)) {
      return persistenceErrorResponse(error, "Failed to persist need-more-info decision.");
    }
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Unable to record need-more-info decision." },
      { status: 400 }
    );
  }
  if (!updated) {
  ...
  let responseSubmission: ProjectSubmission = updated;
  if (resolvedStage === "PROJECT_MANAGER") {
      ...
      movedToChangeReview = await updateSubmission(
        id,
        {
          ...
          workflow: {
            ...
            lockReason: "Project manager requested changes before accepting assignment."
          }
        },
        {
          audit: {
            action: "STATE_CHANGE",
            note: "Project Manager requested changes. Transitioned to LIVE/CHANGE_REVIEW.",
            actorName: principal.name ?? "Approver",
            actorEmail: principal.email ?? undefined
          }
        }
    ...
    try {
      reconciled = await reconcileSubmissionWorkflow(id, {
        actorName: principal.name ?? "Approver",
        actorEmail: principal.email ?? undefined,
        reason: `Need-more-info recorded for stage ${resolvedStage}.`
      });
    } catch (error) {
      return persistenceErrorResponse(error, "Failed to reconcile workflow after need-more-info.");
    }
    if (reconciled) {
  ...
  try {
    await appendGovernanceAuditLog({
      area: "WORKFLOW",
      action: "NEED_MORE_INFO_PROJECT_STAGE",
      ...
      details: `Need-more-info recorded for stage ${resolvedStage}.`,
      metadata: {
        stage: resolvedStage,
        actingAs
      }
