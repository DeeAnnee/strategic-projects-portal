...
import { buildRawExport } from "@/lib/reporting/exports";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const csv = buildRawExport(run, parsed.data.mode ?? "raw");
    const filename = `${run.reportTitle.toLowerCase().replace(/[^a-z0-9]+/g, "-") || "report"}-${new Date()
      .toISOString()
      .slice(0, 10)}-${parsed.data.mode ?? "raw"}.csv`;
    ...
    const message = error instanceof Error ? error.message : "Unable to generate raw export.";
