...
import { shareTemplate } from "@/lib/reporting/service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await shareTemplate(access.principal, id, parsed.data);
    return NextResponse.json({ data: updated });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to share template.";
